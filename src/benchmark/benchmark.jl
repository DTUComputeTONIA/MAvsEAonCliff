...
function resultpvalue(df::DataFrames.DataFrame)
    ...
    problem_size = keys(groupby(df, :ProblemParam_n))
    allowmissing!(df)

    for item in problem_size
        println("Problem Size: ", item["ProblemParam_n"])
        data2 = groupby(
            filter(row -> row["ProblemParam_n"] == item["ProblemParam_n"] && !ismissing(row["runtime"]), df),
            :algorithm,
        )
        for item in data2
            for item2 in data2
...
function runtimes(jobs::Array{Job,1})
    ...
    for job in jobs
        trace = job.trace
        if !isnothing(trace) && job.status == finished
            thing = (
                seed = trace.seed,
    ...
    for job in shuffle(jobs)
        trace = job.trace
        if  job.status == finished && !isnothing(trace) #&& job.status == finished && size(trace.rows)[1]>1
            paramgroups = (
                ProblemParam = deepparameters(trace.individual.problem),
                AlgorithmParams = deepparameters(trace.algorithm),
            )
            # println(job.algorithm.name)
            # println(paramgroups)
...
function runtimesold(jobs::Array{Job,1})
    result = DataFrames.DataFrame()
    allowmissing!(result)
    for job in jobs
        trace = job.trace
...
runtimesold(exp::Experiment) = runtimes(exp.jobs)